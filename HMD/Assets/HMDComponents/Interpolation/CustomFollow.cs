...
namespace UnityEngine.XR.Interaction.Toolkit.UI
{
    ...
    /// </summary>
    [AddComponentMenu("XR/Custom Follow", 22)]
    public class CustomFollow : MonoBehaviour
    {
        ...
        void UpdateRotation(Quaternion rotation)
        {
            if (applyTargetInLocalSpace) 
            {
                Quaternion interp = Quaternion.Slerp(transform.localRotation, rotation, 0.1f);
                transform.localRotation = interp;
                //transform.localRotation = rotation;

            } 
            else
            {
                Quaternion interp = Quaternion.Slerp(transform.rotation, rotation, 0.1f);
